...
@router.get("/health")
def governance_health(_: str = Depends(require_roles(*ADMIN_ONLY))):
    return gov.governance_health()


@router.get("/decisions")
def list_decisions(limit: int = 100, _: str = Depends(require_roles(*ADMIN_ONLY))):
    limit = max(1, min(limit, 1000))
    return {"decisions": gov.get_all_decisions(limit=limit)}


@router.get("/decisions/{claim_id}")
def get_chain(claim_id: str, _: str = Depends(require_roles(*ADMIN_ONLY))):
...
def log_decision(req: DecisionRequest, _: str = Depends(require_roles(*ADMIN_ONLY))):
    try:
...
@router.get("/bias")
def get_bias(_: str = Depends(require_roles(*ADMIN_ONLY))):
    return gov.get_bias_report()


@router.post("/bias")
def record_bias(req: BiasProxyRequest, _: str = Depends(require_roles(*ADMIN_ONLY))):
    try:
...
def bias_evaluation(_: str = Depends(require_roles(*ADMIN_ONLY))):
...
def list_model_cards(_: str = Depends(require_roles(*ADMIN_ONLY))):
    cards = gov.list_model_cards()
    return {"model_cards": cards, "total": len(cards)}


@router.get("/model-cards/{agent_id}")
def get_model_card(agent_id: str, _: str = Depends(require_roles(*ADMIN_ONLY))):
...
def list_state_addenda(_: str = Depends(require_roles(*ADMIN_ONLY))):
    items = gov.list_state_addenda()
    return {"state_addenda": items, "total": len(items)}


@router.get("/state-addenda/{state}")
def get_state_addendum(state: str, _: str = Depends(require_roles(*ADMIN_ONLY))):
...
def generate_adverse_action(req: AdverseActionRequest, _: str = Depends(require_roles(*ADMIN_ONLY))):
